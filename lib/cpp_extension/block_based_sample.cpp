...
void load_and_sample(std::vector<std::vector<MatrixEntry>>& matrix1, 
    ...
    for (int i = 0 ; i < matrix1.size() ; i++){
        if (matrix1[i][block_id].head != -1){
            ...
            if (node_full_length <= 8) { 
                mask1 = (1 << node_full_length) - 1;
            }else if (node_full_length <= sample_size) { 
                mask1 = (1 << 8) - 1;
                mask2 = (1 << node_full_length - 8) - 1;  
            } /*else if (LPflag) {
                node_full_length = BLOCKSIZE/sizeof(int64_t)-2000;
            ...
            if (node_full_length <= 8) { 
                __m512i loaded_values = _mm512_maskz_loadu_epi64(mask1, node_adjlist_ptr);
                for (int j = 0; j < node_full_length; j++) {
                    c = ((int64_t*)&loaded_values)[j]; 
                ...
                __m512i loaded_values1 = _mm512_maskz_loadu_epi64(mask1, node_adjlist_ptr);
                for (int j = 0; j < 8; j++) {
                    c = ((int64_t*)&loaded_values1)[j];
                    tbb::concurrent_hash_map<int64_t, int>::accessor accessor;
                    if (n_id_map[i].insert(accessor, c)) {
                        current_n_id_index = n_ids_index[i].fetch_add(1);
                        accessor->second = current_n_id_index;
                ...
                __m512i loaded_values2 = _mm512_maskz_loadu_epi64(mask2, node_adjlist_ptr);
                for (int j = 0; j < node_full_length - 8; j++) { 
                    c = ((int64_t*)&loaded_values2)[j];
                    tbb::concurrent_hash_map<int64_t, int>::accessor accessor;
                    if (n_id_map[i].insert(accessor, c)) {
                        current_n_id_index = n_ids_index[i].fetch_add(1);
                        accessor->second = current_n_id_index;
                ...
                if (node_full_length <= 8) { 
                    ...
                    for (int j = 0; j < node_full_length; j++) {
                        ...
                        if (n_id_map[i].insert(accessor, c)) { 
                            current_n_id_index = n_ids_index[i].fetch_add(1);
                            accessor->second = current_n_id_index;
                            n_ids[i][current_n_id_index] = c; //gather or load
                        } else {
                            current_n_id_index = accessor->second;
                        }
                        cols[i][batch_location].push_back(current_n_id_index); 
...
void process_by_block_async(std::vector<std::vector<MatrixEntry>>& matrix1, 
    ...
    //std::cout << "null_blocks.size() =" << null_blocks.size() << std::endl;
...
bb_sample(std::string topology_path, std::vector<torch::Tensor> batches, std::vector<int64_t> num_neighbors, int64_t batch_size, std::string dataset, int buffer_size, int num_workers) { 
    ...
    tbb::parallel_for(0, batchNum, [&](int i) {
        ...
        n_ids_index[i].store(batchSize+1);
        n_ids[i].resize(1000000); 
    });
...
int bb_gather(std::vector<torch::Tensor> out_n_ids, int maxValue, int feature_dim, int buffer_size, int num_workers, std::string feature_path) {
    ...
    int flag = gather(Batches, maxValue, GPU_feature_q, CPU_feature_q, feature_q_index, storage_q_index, batch_trashcan, trashcan_count, feature_GCS, feature_dim, buffer_size, num_workers, feature_path, BFS_index); 
    GCS_flag = flag;
    return 0;
}
...
torch::Tensor gather_sagnn(std::string feature_file, torch::Tensor idx, int64_t feature_dim, int i, int num_workers){
    ...
    if (odd == 0){
        ...
        auto options = torch::TensorOptions()
            ...
            .device(torch::kCPU)
            .requires_grad(false);
        auto result = torch::from_blob(batch_feature_queue1.data(), {N, feature_dim}, options);
        ...
        auto options = torch::TensorOptions()
            ...
            .device(torch::kCPU)
            .requires_grad(false);
        auto result = torch::from_blob(batch_feature_queue2.data(), {N, feature_dim}, options);
        return result;
    }
}
...
bool read_BFS() {
    const std::string file_path = "/home/nvme1/Yahoo1M_64bit/Yahoo_BFS_oldnew_idxlist.bin"; 
    ...
    BFS_index.reserve(file_size / sizeof(uint64_t));

    for (std::size_t i = 0; i < file_size; i += sizeof(uint64_t)) {
        uint64_t value = *reinterpret_cast<uint64_t*>(&buffer[i]);
        BFS_index.push_back(value);
    }

    std::cout << "BFS_read done" << std::endl;
    return true; 
}
...
PYBIND11_MODULE(sample, m) {
    ...
    m.def("read_BFS", &read_BFS, "read_BFS");
}
