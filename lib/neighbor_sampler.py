...
def sample_by_block(exp_name, node_idx: Tensor, sizes: List[int], batch_size):
    num_batches = (len(node_idx) + batch_size - 1) // batch_size
    split_node_idx = [node_idx[i*batch_size:(i+1)*batch_size].tolist() for i in range(num_batches)]
    sample.bb_sample(split_node_idx, sizes, batch_size)
